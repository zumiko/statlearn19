...
```{r}
mtrain <- glm(default ~ balance, data = Default_train,
          family = binomial)

d_n_train <- my_lda(Default_train$balance, pi_n, mu_n, sig_sq)
d_y_train <- my_lda(Default_train$balance, pi_y, mu_y, sig_sq)


my_log_pred_train <- ifelse(mtrain$fit < 0.5, "No", "Yes")
my_lda_pred_train <- ifelse(d_n > d_y, "No", "Yes")

conf_lda_train <- table(my_lda_pred_train, Default$default)
conf_lda_train

conf_log_train <- table(my_log_pred, Default$default)
conf_log_train


(1/nrow(Default_test)) * (conf_lda_train[2, 1] + conf_lda_train[1, 2])

(1/nrow(Default_test)) * (conf_log_train[2, 1] + conf_log_train[1, 2])
