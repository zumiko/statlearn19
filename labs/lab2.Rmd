...
```{r}
meanstations = mean(quakes$stations)
#meanstations
stdstations = sd(quakes$stations)
#stdstations

#sigma = sqrt(m1$res/(n1-2))
#ysim <- rnorm(n = n1, mean = f_hatsim(xsim), sd = stdstations)
rss <- sum((x1 - mean(x1))^2)
mean(rss)
#rss1 = (m1$res)^2
#rss1
sigma = rss/(1000-2)
mean(sigma)
error <- rnorm(n = 1000, mean = 0, sd = stdstations)
mean(error)
#sd(error)
...
In the original data there are far more observations around the lower magnitudes. Additionally, I could account for the fact that the standard deviation might not be the same across all x's if the model is not perfectly linear by making an average deviation function as we did with y hat. 
